...
2. **本地 commit**；**push 最多试一次**，失败就停手（见 §10）。
3. 客户端 → 「配置 Profiles」→ **右键当前订阅 → 删除** → 再「添加配置」粘贴**同一订阅链接**（全量重新下载）。
4. **VPN 开关关一次再开一次** → **重选被重置的节点** → **按 §4 验证**。

> 只点「同步 / 更新订阅」不重建规则数据缓存（见 §2、§6）。
...
> **最重要的一条**：**往 GitHub push 成功 ≠ 客户端生效。** 中间隔着本地 file 缓存 + 独立刷新周期。

---

## 3. 三条绝对不要做的事（血泪教训）

1. ❌ **绝不要直接编辑** 缓存里的 `provider-*.txt` → 内容与 manifest 的 sha256 对不上 → 该规则集判定损坏 → **`ruleCount` 归零（规则全失效）**，且 **API 无法恢复**，只能靠 GUI 重载或删除重加 profile。
2. ❌ **不要改 `manifest.json` 的 sha256** 绕过校验 → 同为 hack，上层可能还有校验，风险更高。
3. ❌ **不要指望 Clash API 刷新 file 类型 provider** → `PUT /providers/rules/<name>` 返回 **HTTP 204** 是**假成功**：`file` 类型没有 url 可重拉，只会重读本地文件；若文件已被手改，反而直接把规则集清空。

> 正确做法只有一个：**让客户端自己重新下载**（§0 第 3 步）。
...
**结论**：这套 API 只能**读诊断信息**，不能修复/刷新 file 类型 provider。
...
1. **独立刷新周期**：每个 provider 有 `interval`（本仓库统一 **90 天** = `7776000` 秒），客户端不自动拉。
2. **`x-hako-side-update-safe` 标记**：让「同步 / 更新订阅」**跳过** provider 重新下载 → 只 reload 配置，数据还是旧的。

**推论（实测）**：只开关 VPN 开关**不会**重新拉取（开关 = 启停隧道 + 重载本地已解析配置与本地 provider 缓存，不重解析订阅）；「备份恢复」也**不行**（只还原 config 文本，不重建 provider 缓存）。**必须「删除 profile → 从同一链接重新添加」。**
...
- [ ] 客户端里**删除并重新添加** profile（不是只点「同步」）→ VPN 开关关/开各一次
...
- 本机 GitHub 偶发 `SSL_ERROR_SYSCALL` / 连不上：**本地 commit 必做，push 试一次，失败即停手**，不反复重试、不改 remote、不换 SSH、不加跳过校验的环境变量。
...
> Hako 容器目录名以实际安装为准（`ls "$HOME/Library/Group Containers" | grep -i hako`）；profile/revision 的 UUID 会随「删除重加」变化，属正常。
